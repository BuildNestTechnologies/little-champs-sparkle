...
export const About = () => (
  <section id="about" className="relative py-24 lg:py-32 overflow-hidden">
    <div className="absolute -top-20 right-0 w-[420px] h-[420px] rounded-full bg-gradient-sunshine opacity-30 blur-3xl" />
    <div className="container relative grid lg:grid-cols-2 gap-14 items-center">
      <motion.div
        ...
        className="relative"
      >
        <div className="absolute -top-8 -left-8 w-40 h-40 rounded-3xl bg-gradient-candy -rotate-6 opacity-80" />
        <div className="absolute -bottom-8 -right-8 w-40 h-40 rounded-3xl bg-gradient-sky rotate-6 opacity-80" />
        <div className="relative rounded-[2.5rem] overflow-hidden shadow-pop ring-4 ring-white">
          <img src={aboutImg} alt="Cozy classroom at Little Champs" className="w-full h-auto" loading="lazy" />
        </div>
        <div className="absolute -bottom-6 left-6 bg-white rounded-2xl shadow-card px-4 py-3 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-sunshine flex items-center justify-center">
            <Sparkles className="w-5 h-5 text-ink" />
          </div>
          <div>
            <div className="font-display font-bold text-ink leading-tight">Since 2019</div>
            <div className="text-xs text-ink/60">Loved by parents</div>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <h2 className="mt-2 font-display font-bold text-4xl lg:text-5xl text-ink leading-tight">
          A loving home where <span className="gradient-text">little champs</span> bloom.
        </h2>
        <p className="mt-5 text-lg text-ink/70 font-body">
        ...
        <p className="mt-3 text-ink/70 font-body">
        ...
        <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-4">
          {STATS.map((s) => (
            <div key={s.label} className="rounded-2xl bg-white shadow-card p-4 text-center">
              <div className={`font-display font-bold text-3xl ${s.color}`}>
                <Counter end={s.value} suffix={s.suffix} />
              </div>
              <div className="text-xs font-playful font-bold text-ink/70 uppercase tracking-wide mt-1">{s.label}</div>
            </div>
          ))}
        </div>

        <a href="#programs" className="mt-8 inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-ink text-white font-bold shadow-card btn-pop">
          Read Our Story
        </a>
      </motion.div>
    </div>
