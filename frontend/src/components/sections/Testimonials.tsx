...
export const Testimonials = () => {
  ...
  return (
    <section id="testimonials" className="relative py-24 lg:py-32 bg-gradient-hero dark:bg-background pattern-clouds overflow-hidden transition-colors duration-400">
      <div className="container relative">
        <div className="text-center max-w-2xl mx-auto mb-14">
          ...
          <div className="mt-3 flex items-center justify-center gap-1">
            ...
            <span className="ml-2 font-playful font-bold text-ink dark:text-foreground">5.0 from 6 reviews</span>
          </div>
        </div>
        ...
        >
            ...
            >
              <div className="relative h-full bg-white dark:bg-card rounded-2xl shadow-pop p-7 sm:p-10 flex flex-col">
                ...
                <div className="mt-auto pt-6 flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-display font-bold text-ink dark:text-foreground">{r.name}</div>
                    <div className="text-xs text-ink/60 dark:text-foreground/60 font-playful">{r.when}</div>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          <button onClick={() => setI((v) => (v - 1 + REVIEWS.length) % REVIEWS.length)} aria-label="Previous review" className="absolute -left-3 sm:-left-12 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-white dark:bg-card shadow-card flex items-center justify-center text-ink dark:text-foreground z-10">
            <ChevronLeft className="w-5 h-5" />
          </button>
          <button onClick={() => setI((v) => (v + 1) % REVIEWS.length)} aria-label="Next review" className="absolute -right-3 sm:-right-12 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-white dark:bg-card shadow-card flex items-center justify-center text-ink dark:text-foreground z-10">
        ...
        <div className="flex justify-center gap-2 mt-8">
          {REVIEWS.map((_, k) => (
            <button
              ...
              className={`h-2.5 rounded-full transition-all ${k === i ? "w-8 bg-candy" : "w-2.5 bg-ink/20 dark:bg-foreground/20 hover:bg-ink/40 dark:hover:bg-foreground/40"}`}
            />
          ))}
        </div>
      </div>
